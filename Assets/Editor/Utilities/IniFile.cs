...
public class IniFile
{
        ...
        while (!oReader.EndOfStream)
        {
            ...
            if (line != string.Empty)
            {
                ...
                if (regexcomment.Match(line).Success)
                {
                    ...
                    //Debug.Log(string.Format("Skipping Comment: {0}", m.Groups[0].Value));
                }
                else if (regexsection.Match(line).Success)
                {
                    m = regexsection.Match(line);
                    //Debug.Log(string.Format("Adding section [{0}]", m.Groups[1].Value));
                    tempsection = AddSection(m.Groups[1].Value);
                }
                else if ( regexkey.Match(line).Success && tempsection != null)
                {
                    m = regexkey.Match(line);
                    //Debug.Log(string.Format("Adding Key [{0}]=[{1}]", m.Groups[1].Value, m.Groups[2].Value));
                    tempsection.AddKey(m.Groups[1].Value).Value = m.Groups[2].Value;
                }
                else if ( tempsection != null )
                {
                    //  Handle Key without value
                    //Debug.Log(string.Format("Adding Key [{0}]", line));
                    tempsection.AddKey(line);
                }
                else
                {
                    //  This should not occur unless the tempsection is not created yet...
                    //Debug.Log(string.Format("Skipping unknown type of data: {0}", line));
                }
            }
        }
        oReader.Close();
    ...
    public class IniSection
    {
        ...
        public IniKey AddKey(string sKey)
        {
            ...
            //if (sKey.Length != 0) //***PLAYSIDE CHANGE
            {
